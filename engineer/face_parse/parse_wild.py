...
def evaluate(data_dir, ignore_str, cp='79999_iter.pth'):
    
    ignore_list = ignore_str.split(' ')
    print(ignore_list)
    image_all_list = sorted(glob.glob(data_dir + "*.jpg")) + \
                     sorted(glob.glob(data_dir + "*.png")) + \
                     sorted(glob.glob(data_dir + "*.bmp"))
    
    image_list = []
    for fn in image_all_list:
        ignored = False
    ...
    with torch.no_grad():
        for i in tqdm.trange(0, len(image_list)):
            ...
            Image.fromarray(vis_im_label).save(image_path[:-4]+"_semask.png")

def evaluate_single(img, cp='79999_iter.pth'):
    
    n_classes = 19
    net = BiSeNet(n_classes=n_classes)
    net.cuda()
    save_pth = osp.join('./checkpoints/face_parsing/', cp)
    net.load_state_dict(torch.load(save_pth))
    net.eval()
...
if __name__ == "__main__":
    ...
    parser.add_argument('--dir', type = str, 
                        help = 'all images in this dir will be parsed, excluding ignored suffix')
    parser.add_argument('--ignore', type = str, 
                        default = "_semask.png _norm.png _norm_front.png _norm_back.png", 
                        help = 'files with these as suffix will be ignored')
    opt = parser.parse_args()
    
    evaluate(opt.dir, opt.ignore)
